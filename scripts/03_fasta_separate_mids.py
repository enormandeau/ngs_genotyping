...
if __name__ == "__main__":
    ...
    if len(opts) == 0:
        short_help("No options specified")
        sys.exit(0)
    ...
    mids = [x.strip() for x in open(mid_file, "r").readlines() if x.strip() != ""]
    files = {}
    
    for m in mids:
        files[m] = open("trimmed_separated_sequences/" + m + ".fasta", "w")
    ...
    for s in fasta_sequences:
        ...
        try:
            ...
            print "Output error: Make sure that your MID tags are EXACTLY the \
same\nin your 'midnames.txt' file and your separated fasta files"
