...
for file in $(ls -1 ./individual_summary/*summary); do echo Treating file $file; tot=$(perl -pe 's/^ +//' $file | cut -d " " -f 1 | total); cat $file | while read line; do value=$(echo $line | perl -pe 's/^ +//' | cut -d " " -f 1); allele=$(echo $line | perl -pe 's/^ +//' | cut -d " " -f 2); prop=$(echo $value / $tot | bc | cut -c -7); echo $prop $allele; done > $file".proportions"; done
...
ls -1 individual_summary/*.proportions > summary_file_names_proportions.txt
